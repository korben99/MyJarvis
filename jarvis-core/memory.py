...
def store_memory_vector(user_code: str, entry: dict):
    ...
    try:
        ...
        if summary:
            ...
            text = (
                f"User said: {entry['user']}\n"
                f"Assistant replied: {entry['assistant']}\n"
                f"Mood: {entry['mood']}\n"
...
def get_self_memory() -> dict:
    """Get Jarvis's self-knowledge."""
    try:
        with open(SELF_MEMORY_PATH, "r") as f:
            return json.load(f)
...
def save_self_memory(data: dict):
    """Save self memory to disk."""
    os.makedirs(os.path.dirname(SELF_MEMORY_PATH), exist_ok=True)
    with open(SELF_MEMORY_PATH, "w") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)


def add_self_learning(learning: str):
    """Add a learning to Jarvis's growth log."""
